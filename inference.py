...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='VisemeNet inference')
    parser.add_argument('--input', nargs='?', type=str,
                        help='Input .wav filename(assume file is located at `data/test_audio/`.')
    parser.add_argument('--output', nargs='?', type=str,
                        help='Output estimated mayaparam.txt filename.')
    ...
    # feedforward testing
    graph = load_graph('./frozen/visemenet_frozen.pb')
    test_frozen(graph=graph, test_audio_name=test_audio_name[:-4])
    print('Finish forward testing.')

    # output viseme parameter
    output_filepath = eval_viseme(os.path.splitext(test_audio_name)[0])

    shutil.move(output_filepath, args.output)
    print('Done. output filename = ', args.output)
